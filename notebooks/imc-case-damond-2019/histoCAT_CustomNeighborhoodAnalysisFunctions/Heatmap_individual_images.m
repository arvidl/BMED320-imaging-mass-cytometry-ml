...
%%%%%%%%%% FOR PANCREAS START
% Modifiy order based on pseudotime
sessionData= retr('sessionData');
Gates_pseudotime = {};
Gates_selected = gates(selectedall_gates',1);
Gates_unique_pseudotime = unique(sessionData(:,111),'stable');
for i=1:size(Gates_selected,1)
    Gates_pseudotime{i,1} = Gates_selected{i,1};
    Gates_pseudotime{i,2} = Gates_unique_pseudotime(i,1);
    Gates_pseudotime{i,3} = Matrix_Delta_cut_noNaN(i,:);
end
Sorted_Gates_pseudotime = sortrows(Gates_pseudotime,2);
Sorted_gate_names = Sorted_Gates_pseudotime(:,1);
Sorted_Matrix_Delta_cut_noNaN = cell2mat(Sorted_Gates_pseudotime(:,3));
% Clustergram for all images
Delta = clustergram(Sorted_Matrix_Delta_cut_noNaN,'RowLabels',Sorted_gate_names...
    ,'ColumnLabels',Unique_all_string_names_cut,...
    'Cluster','row','linkage','ward','Colormap',redbluecmap); addTitle...
    (Delta,['Deltaclustergram for all images with Pixel',...
    num2str(pixelexpansion),'_Perm_',num2str(permutations),'_',pheno_name,'_',Extra_information]);
plot_delta =plot(Delta);
%%%%%%%%%% FOR PANCREAS END
